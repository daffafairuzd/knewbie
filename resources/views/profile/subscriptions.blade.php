                            ...
                            <div class="space-y-5">
                                @foreach($transactions as $transaction)
                                    @php
                                        ...
                                        $statusColor = $isActive ? 'text-[#0073FF]' : 'text-red-500';

                                        $dateLabelPrefix = $isActive ? 'active until' : 'expired on';

                                        $dateText = $transaction->ended_at
                                            ? $transaction->ended_at->translatedFormat('j F')
                                            : '-';
                                    @endphp
                                    ...
                                    <a
                                        ...
                                        class="block rounded-[26px] bg-white shadow-md hover:shadow-lg transition
                                               px-6 py-4 md:px-8 md:py-5 border border-gray-100"
                                    >
                                        <div class="flex items-center justify-between gap-4">
                                            <div>
                                                <p class="font-semibold text-base md:text-lg text-gray-900">
                                                    {{ $planName }}
                                                </p>
                                                <p class="mt-1 text-sm text-gray-500">
                                                    {{ $dateLabelPrefix }} {{ $dateText }}
                                                </p>
                                            </div>

                                            <span class="text-sm md:text-base font-semibold {{ $statusColor }}">
                                                {{ $statusText }}
                                            </span>
                                        </div>
                                    </a>
